...
function checkTC(giveaway, encc, frontpage){
	//USING STEAMAPI
	var t = $(giveaway).find(".global__image-outer-wrap--game-medium").find(".global__image-inner-wrap")
	var appid = $(t).css("background-image");
	if (appid == null){
    ...
    xhr.onreadystatechange = function () {
		if (xhr.readyState == 4 && xhr.status == 200){
			var jsonResponse = JSON.parse(this.responseText);
			if (jsonResponse[appid].success == false && !encc){
				checkTC(this, true, frontpage); //try with cc = en
			}else{
				for (var i = 0; i < jsonResponse[appid].data.categories.length; i++){
					var categorie = jsonResponse[appid].data.categories[i];
					if (categorie.id == 29){
						if (frontpage){
							//$(giveaway).parent().remove();
							$('<input type="button" value="Join" class="btnSingle" walkState="join" style="width:130px; height:80px; background-color:#BCED91;">').appendTo($(giveaway).parent());							
							return true;
						}
						else{
							//var linkToGiveaway = $(giveaway).find(".giveaway__heading__name").attr("href");
							//$(giveaway).parent().remove();
							//$('#posts').find("[href='" + linkToGiveaway + "']").parent().parent().remove();
							$('<input type="button" value="Join" class="btnSingle" walkState="join" style="width:130px; height:80px; background-color:#BCED91;">').appendTo($('#posts').find("[href='" + linkToGiveaway + "']").parent().parent());
							return true;
						}
					}
				}
